...
export function useBatchList(params: Record<string, string | number> = {}, options?: { enabled?: boolean }) {
  ...
  const { data, error, mutate, isLoading, isValidating } = useSWR<{
    data: PenyaluranBatch[]; hasMore: boolean; total: number; page: number; limit: number;
  }>(key, fetcher, LIST_SWR_OPTIONS);

  const responsePage = data?.page ?? 0;

  return {
    data: data?.data ?? [], hasMore: data?.hasMore ?? false, total: data?.total ?? 0,
    page: responsePage, requestedPage,
...
export function useAnakGridList(params: Record<string, string | number> = {}, options?: { enabled?: boolean }) {
  ...
  const { data, error, mutate, isLoading, isValidating } = useSWR<{
    data: PenyaluranRow[]; hasMore: boolean; total: number; page: number; limit: number;
  }>(key, fetcher, LIST_SWR_OPTIONS);
